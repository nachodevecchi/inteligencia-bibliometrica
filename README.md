...
El repositorio contiene tres módulos. El Módulo 0 es un **prerequisito del Módulo 1**. El Módulo 2 es independiente y puede usarse directamente con el Excel exportado de Scopus, sin pasar por el Módulo 0.
...
- **Descubrimiento de verticales industriales** — identifica clústeres temáticos emergentes de la base de papers y los asocia a sectores industriales mediante clustering semántico
...
## Módulo 0 — Enriquecimiento de la base (PREREQUISITO para el Módulo 1)

> ⚠️ **Este módulo debe ejecutarse antes de usar el Módulo 1.** El Módulo 2 no lo requiere.
...
> 💡 El archivo resultante (`BASE ENRIQUECIDA.xlsx`) es el que usa el Módulo 1.
...
## Módulo 2 — Descubrimiento de Verticales Industriales por Clustering Semántico

### ¿Qué hace?

A partir del Excel exportado de Scopus, identifica automáticamente clústeres temáticos de investigación y los asocia a verticales industriales. A diferencia del Módulo 1, **no requiere el Módulo 0**: trabaja directamente con títulos, abstracts y keywords de los papers.

El enfoque es bottom-up: las verticales **emergen de los datos**, no se definen de antemano. El único paso manual es leer los clústeres generados y asignarles un nombre.

**Flujo del análisis:**
1. Cada paper → vector semántico (embedding de título + abstract + keywords)
2. UMAP reduce la dimensionalidad (384D → 10D para clustering, 2D para visualización)
3. HDBSCAN encuentra clústeres naturales sin que definas cuántos
4. TF-IDF extrae las keywords más representativas de cada clúster
5. Vos revisás los clústeres y les asignás nombre y vertical industrial
6. Se exporta un Excel con cada paper etiquetado y un mapa de conocimiento 2D

**Outputs generados:**
- `Clusters_Descubiertos.xlsx` — Excel con 3 hojas: dataset completo etiquetado, diccionario de clústeres con keywords, y resumen por vertical
- `mapa_conocimiento_MiUniversidad.png` — mapa 2D donde cada punto es un paper, coloreado por vertical industrial
- `embeddings_papers.npy` — embeddings guardados para no recalcular en futuras corridas

### Archivos necesarios

| Archivo | Descripción | Dónde conseguirlo |
|---|---|---|
| `base_scopus.xlsx` | Base de papers exportada de Scopus | Exportá desde Scopus |

> 💡 El archivo de Scopus puede usarse tal cual, sin pasar por el Módulo 0.

### Cómo usar el notebook

#### Paso 1 — Abrir el notebook

Abrí Jupyter Notebook y navegá hasta `clustering_verticales/`. Abrí el archivo `descubrir_verticales_clustering.ipynb`.

#### Paso 2 — Instalar dependencias (primera vez)

Ejecutá la **Celda 1**. Instala automáticamente todas las librerías necesarias. Tarda 2-3 minutos la primera vez.
...
|---|---|---|
| Celda 3 | Carga los datos y construye el texto de cada paper | < 1 min |
| Celda 4 | Calcula los embeddings semánticos | 10-15 min en CPU |
| Celda 5 | UMAP + HDBSCAN: reduce dimensionalidad y genera clústeres | 3-5 min |
| Celda 6 | Extrae las keywords más representativas de cada clúster | < 1 min |

> 💡 Si ya calculaste los embeddings antes, podés saltear la Celda 4 y usar la **Celda 4b** para cargarlos desde el archivo `embeddings_papers.npy`. Esto ahorra 10-15 minutos en corridas posteriores.

#### Paso 5 — Nombrar los clústeres (único paso manual)

La **Celda 7** imprime la información de cada clúster: cantidad de papers, keywords principales y títulos de ejemplo. Leé el output, luego completá el diccionario `NOMBRES_MANUALES` en la misma celda:

```python
NOMBRES_MANUALES = {
    0:  {'nombre_corto': 'Machine Learning & NLP',     'vertical_industrial': 'AI & ML'},
    1:  {'nombre_corto': 'Oncología Molecular',         'vertical_industrial': 'Pharma & Drug Discovery / Biotech'},
    2:  {'nombre_corto': 'Energías Renovables',         'vertical_industrial': 'Energía & Cleantech'},
    # ... un ID por cada clúster que apareció en el output
}
```

Podés usar `/` para asignar más de una vertical a un mismo clúster. Las verticales sugeridas son: `AI & ML`, `Biotech`, `Pharma & Drug Discovery`, `Diagnóstico & MedTech`, `Neurotecnología`, `Energía & Cleantech`, `Agritech`, `Foodtech`, `Materiales Avanzados`, `Quantum Tech`, `Space Tech`, `Climate Tech`, `Geociencias & Minería`, `Conservación & Ecotech`, `Social Tech`, `HPC & Big Data`, `Matemática Aplicada`.

#### Paso 6 — Generar visualización y exportar

Ejecutá las celdas 8, 9 y 10 en orden para generar el mapa de conocimiento, exportar el Excel y ver el resumen final.

#### ¿Cuántos clústeres esperar?

Depende del tamaño de la base y del parámetro `MIN_CLUSTER_SIZE` (configurable en la Celda 2):

| Tamaño de la base | `MIN_CLUSTER_SIZE` sugerido | Clústeres esperados |
|---|---|---|
| ~3.000 papers | 30 | 20-35 |
| ~5.000 papers | 50 | 25-40 |
| ~10.000 papers | 80 (por defecto) | 35-55 |
| ~20.000+ papers | 120 | 40-60 |

Si el resultado tiene demasiados clústeres muy pequeños, subí `MIN_CLUSTER_SIZE` y volvé a correr desde la Celda 5 (los embeddings no se recalculan, tarda 1-2 minutos).
...
├── .gitignore
│
├── enriquecimiento/             ← Módulo 0: Enriquecimiento de la base (prerequisito del Módulo 1)
...
│
└── clustering_verticales/   ← Módulo 2: Descubrimiento de verticales por clustering semántico
    └── descubrir_verticales_clustering.ipynb
```
